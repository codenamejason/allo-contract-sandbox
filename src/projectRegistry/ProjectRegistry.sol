...
contract ProjectRegistry is Initializable {
    ...
    //     count: 3,
    //     list: {
    //         _OWNERS_LIST_SENTINEL => owner1Address,
    //         owner1Address => owner2Address,
    //         owner2Address => owner3Address,
    //         owner3Address => _OWNERS_LIST_SENTINEL
    //     }
    // }
    struct OwnerList {
        uint256 count;
    ...
    function addProjectOwner(uint256 projectID, address newOwner) external onlyProjectOwner(projectID) {
        require(newOwner != address(0) && newOwner != _OWNERS_LIST_SENTINEL && newOwner != address(this), "PR001");
     ...
     * @param projectID ID of project
     */
    function _initProjectOwners(uint256 projectID) internal {
        OwnerList storage owners = projectsOwners[projectID];
